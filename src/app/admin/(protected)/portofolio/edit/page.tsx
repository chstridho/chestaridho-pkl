...
type Project = {
  ...
  image?: string | null;      // URL public untuk preview
  langs?: string[] | null;
  repo?: string | null;
  demo?: string | null;
  year?: number | null;
...
export default function EditProjectPage() {
  ...
  const pid = q.get('pid'); // mis. /admin/portofolio/edit?pid=<id or slug>
  ...
  // LANGS INPUT mentah (diparse saat submit/preview)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!pid) throw new Error('Parameter pid tidak ditemukan');

        const res = await fetch(`/api/portfolio/${pid}`, { cache: 'no-store' });
        const ct = res.headers.get('content-type') || '';
        const j = ct.includes('application/json') ? await res.json() : { error: await res.text() };
        if (!res.ok) throw new Error(j?.error || 'Project tidak ditemukan');
        ...
        setForm({
          ...
          image: p.cover_path ? publicStorageUrl(p.cover_path) : '',
          langs: p.techs || [],
          repo: p.repo_url || '',
          demo: p.demo_url || '',
          year: p.year ?? yearNow,
        });
        setLangsInput((p.techs || []).join(', '));
      } catch (e: any) {
  ...
  async function submitDirect() {
    ...
    setSubmitting(true);
    try {
      if (!pid) throw new Error('pid kosong');
      ...
      await toast.promise(
        (async () => {
          ...
          // Tidak kirim published/featured di edit (biarkan nilai sekarang)
          if (file) fd.set('file', file, file.name); // hanya jika ganti cover

          const res = await fetch(`/api/portfolio/${pid}`, { method: 'PATCH', body: fd });
          const ct = res.headers.get('content-type') || '';
          ...
          if (!res.ok) {
            ...
            throw new Error(msg || 'Gagal menyimpan perubahan');
          }
        })(),
        { loading: 'Menyimpan perubahan…', success: 'Perubahan tersimpan!', error: (e) => e.message || 'Gagal menyimpan' }
      );

      router.replace('/admin/portofolio');
    } catch (e: any) {
  ...
  return (
    <div className="theme-surface min-h-dvh relative overflow-hidden">
      ...
      <div className="container-app py-8 sm:py-10">
        ...
        >
          <div>
            <h1 className={`${anton.className} text-3xl sm:text-4xl font-extrabold text-white`}>Edit Project</h1>
            <p className={`${manrope.className} mt-1 text-white/85`}>Perbarui detail project dan simpan (Ctrl/⌘+Enter).</p>
          </div>
        ...
        {/* Body */}
        {loading ? (
          <div className="mt-10 text-white/70">Memuat data…</div>
        ) : (
          <motion.div
          ...
          >
            ...
            >
              ...
              {/* Upload lokal */}
              <div className="grid gap-1.5">
                <label className="text-xs font-semibold uppercase text-white/80">Gambar (upload baru untuk mengganti)</label>
                <label
              ...
              <div className="grid gap-1.5 lg:grid-cols-2 lg:gap-4">
                <div className="grid gap-1.5">
                  <label className="text-xs font-semibold uppercase text-white/80">Demo (opsional)</label>
                  <input
                    value={form.demo || ''}
              ...
              <div className="mt-3 flex items-center justify-end gap-2">
                ...
                <button
                  onClick={(e) => { e.preventDefault(); void submitDirect(); }}
                  disabled={submitting}
                ...
                <div className="rounded-2xl border border-white/20 bg-white/10 p-4 ring-1 ring-white/15">
                  ...
                  <div className="mt-3 relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-neutral-400/60">
                    {preview || form.image ? (
                      ...
                      <img src={preview || (form.image as string)} alt={form.title || 'preview'} className="h-full w-full object-cover" />
                    ) : (
                  ...
                  <div className="mt-3">
                    ...
                    <p className={`${manrope.className} mt-1 text-sm text-white/80`}>{form.description || 'Deskripsi singkat akan tampil di sini.'}</p>
